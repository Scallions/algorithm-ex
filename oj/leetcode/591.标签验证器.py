...
class Solution:
    def isValid(self, code: str) -> bool:
        # return True
        t, l = self.parse_sentence(code, 0)
        return t if l == len(code) else False


    def parse_sentence(self, code, s):
        t, s, tag = self.parse_stag(code, s)
        if not t:
            return False, -1
        # 判断 cdata
        t, s = self.parse_content(code, s)
        if not t:
            return False, -1
        t, s = self.parse_etag(code, s, tag)
        return t, s

    def parse_stag(self, code, s):
        # 检查开头
        l = len(code)
        if s+1 >= l:
            return False, -1, None
        if code[s] != '<' or code[s+1] == '/':
            return False,-1,None
        s = s+1
        c = s
        # 检查 tag name
        while s<l and code[s] != '>' and s < c+9:
            # 是否大写
            if code[s] < 'A' or code [s] > 'Z':
                return False, -1,None
            s += 1
        if s >=l or s == c or code[s] != '>':
            return False, -1,None
        return True, s+1, code[c:s]

    def parse_etag(self, code, s, ptag):
        l = len(code)
        if s+2 >= l:
            return False, -1
        # check head
    ...
    def parse_content(self, code, s):
        l = len(code)
        if s+2 >= l:
            return False, -1
        # 判断是否到etag
        while s<l and code[s:s+2] != "</":
            # 判断是 abc、content、cdata
            if code[s] != '<':
                t, s = self.parse_abc(code, s)
            elif code[s+1] == '!':
                t, s = self.parse_cdata(code, s)
            else:
                t, s = self.parse_sentence(code, s)
            if not t:
                return False, -1
        return True, s

    def parse_abc(self, code, s):
        l = len(code)
        while s<l and code[s]!="<":
            s += 1
        if s >= l:
            return False, -1
        return True, s
