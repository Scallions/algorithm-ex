...
public:
    vector<vector<int>> intervalIntersection(vector<vector<int>>& firstList, vector<vector<int>>& secondList) {
        ...
        for(int i=0; i<n1; ++i){
            while(l2<n2&&firstList[i][0]> secondList[l2][1]){
                ++l2;
            }
            while(l2<n2&&secondList[l2][0]<=firstList[i][1]){
                s = max(firstList[i][0], secondList[l2][0]);
                e = min(firstList[i][1], secondList[l2][1]);
                res.push_back(vector<int>{s, e});
                if(secondList[l2][1]<=firstList[i][1]) ++l2;
                else break;
            }
        }
        return res;
